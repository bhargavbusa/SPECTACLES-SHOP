          ...
          <table id="example1" class="table table-bordered table-hover table-striped">
			<thead>
			    <tr>
			        ...
			        <th style="width: 170px;" >Product Details</th>
                    <th>
                    	Payment Information
                    </th>
                    <th>Paid Amount</th>
            ...
            <tbody>
            	...
            	foreach ($result as $row) {
					...
					<tr class="<?php if($row['payment_status']=='Pending' || $row['order_status']=='Canceled' || $row['order_status']=='Canceling'){echo 'bg-r';}else{echo 'bg-g';} ?>">
	                    <td><?php echo $i; ?></td>
	                    <td>
                        ...
                        <td>
                        <?php
                     $statement11 = $pdo->prepare("SELECT * FROM tbl_customer WHERE cust_id=?");
                     $statement11->execute(array($row['customer_id']));
                     $result11 = $statement11->fetchAll(PDO::FETCH_ASSOC);
                     foreach ($result11 as $row11) {
                    ?> 
                        <b>Address:</b><br><?php echo $row11['cust_s_address']; ?><br>
                            <b>City &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp: </b><?php echo $row11['cust_s_city']; ?><br>
                            <b>State &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp: </b> <?php echo $row11['cust_s_state']; ?><br>
                            <?php
                             $statement22 = $pdo->prepare("SELECT * FROM tbl_country WHERE country_id=?");
                             $statement22->execute(array($row11['cust_s_country']));
                             $result22 = $statement22->fetchAll(PDO::FETCH_ASSOC);
                             foreach ($result22 as $row22) {
                            ?>
                            <b>Country &nbsp: </b> <?php echo $row22['country_name']; ?><br>
                            <?php
                             }
                            ?>
                            <b>Zip Code: </b> <?php echo $row11['cust_s_zip']; ?><br><br>
                            <?php
                     }
                            ?>
                        </td>
                        ...
                        <td>
                        	...
                        	<?php elseif($row['payment_method'] == 'Bank Deposit'): ?>
                                ...
                                <b>Transaction Id:</b> <br><?php echo $row['txnid']; ?><br>
                        	<?php endif; ?>
                        </td>
                        ...
                        <td>
                            ...
                            if($row['payment_status']=='Completed') {
                                if($row['shipping_status']=='Shipping'){
                                    ?>
                                    <a href="shipping-change-status.php?id=<?php echo $row['id']; ?>&task=Shipped" class="btn btn-warning btn-xs" style="width:100%;margin-bottom:4px;">Mark Complete</a>
                                    <?php
                                }
                            }
                            ?>
                            ...
                            <?php
                             if($row['shipping_status']=='Shipped'){
                                if($row['order_status']=='Pending'){
                                    ?>
                                    <a href="delivered-change-status.php?id=<?php echo $row['id']; ?>&task=delivered" class="btn btn-warning btn-xs" style="width:100%;margin-bottom:4px;">Mark Complete</a>                                    <?php
                                }
                             }

                            ?>
                            <?php
                            if($row['order_status']=='Canceling'){
                            ?>
                            <a href="order-canceled-status.php?id=<?php echo $row['id']; ?>&taskid1=Canceled" class="btn btn-warning btn-xs">Cancel Order</a>
                            <?php
                            }
                            ?>
                        </td>
